...
public class DbMappingUtil {
    ...
    public static Object getValue(int sqlType, Class<?> classType, ResultSet rs, int rsIndex, DatabaseMapperImpl databaseMapper) throws SQLException {
        ...
        if (javaValueParser == null) {
            try {
                ...
                throw new DatabaseConnectionException("No se encontró la información para la " + " clase->" + classType.getName() + " sqlType ->" + sqlType + " column name ->" + rs.getMetaData().getColumnType(rsIndex));
            }
        }
    ...
    /**
     * getValueFromJsonConverter
     * @param jpaConverterJson jpaConverterJson
     * @param rs rs
     * @param rsIndex rsIndex
     * @param databaseMapper databaseMapper
     * @return Object
    ...
    public static Object getValueFromJsonConverter(JpaConverterJson<?> jpaConverterJson, ResultSet rs, int rsIndex, DatabaseMapperImpl databaseMapper) throws SQLException {
        ...
        if (javaValueParser == null) {
            throw new DatabaseConnectionException("No se encontró la información para la " + " clase->" + jpaConverterJson.getClass().getName() + " column name ->" + rs.getMetaData().getColumnType(rsIndex));
        }
        return jpaConverterJson.convertToEntityAttribute(javaValueParser.readValue(rs, rsIndex));
    }

    /**
     * getValueFromCustomConverter
    ...
    public static Object getValueFromCustomConverter(BaseConverter attributeConverter, ResultSet rs, int rsIndex, DatabaseMapperImpl databaseMapper) throws SQLException {
        JavaValueParser_Converter javaValueParser = databaseMapper.getJavaValueParserByConverter();
        if (javaValueParser == null) {
            throw new DatabaseConnectionException("No se encontró la información para la " + " clase->" + attributeConverter.getClass().getName() + " column name ->" + rs.getMetaData().getColumnType(rsIndex));
        }
                        ...
                        if (field.isAnnotationPresent(Convert.class)) {
                            ...
                            if (StringUtils.isNotBlank(references)) {
                                ...
                                if (converter != null) {
                                    ...
                                    ReflectionUtil.setAttributeValue(instance, field, getValue(rs.getMetaData().getColumnType(i + 1), field.getType(), rs, i + 1, databaseMapper), false);
                                }
                            } else {
                                ReflectionUtil.setAttributeValue(instance, field, getValue(rs.getMetaData().getColumnType(i + 1), field.getType(), rs, i + 1, databaseMapper), false);
                            }
    ...
    protected static void processConverter(Object instance, Field field, ResultSet rs, DatabaseMapperImpl databaseMapper, int index, Object converter) throws InvocationException, SQLException {
        if (converter instanceof JpaConverterJson) {
            ...
            ReflectionUtil.setAttributeValue(instance, field, getValueFromJsonConverter(jpaConverterJson, rs, index + 1, databaseMapper), false);
        } else {
            BaseConverter attributeConverter = BaseConverter.class.cast(converter);
            ReflectionUtil.setAttributeValue(instance, field, getValueFromCustomConverter(attributeConverter, rs, index + 1, databaseMapper), false);
        }
    }
